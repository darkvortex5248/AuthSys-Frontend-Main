...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // ── New path: Supabase sign-up ─────────────────────────────────
      if (hasSupabaseConfig) {
        const { data, error } = await supabase.auth.signUp({
          email: formData.email,
          password: formData.password,
          options: {
            data: { username: formData.username }, // stored in user_metadata
        ...
        if (data.session) {
          ...
          // Email confirmation required — Supabase sent the verification link.
      ...
      // ── Legacy path: backend register + login ──────────────────────
      if (!turnstileToken) {
