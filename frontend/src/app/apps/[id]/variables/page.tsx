  ...
  return (
    <DashboardLayout>
      ...
      <div className="flex gap-4 mb-8 bg-zinc-900 p-4 rounded-xl border border-zinc-800 shadow-md">
        <Input placeholder="Variable Name (e.g. GAME_VERSION)" value={name} onChange={e => setName(e.target.value)} className="bg-zinc-950 border-zinc-800 text-white w-1/3 focus-visible:ring-indigo-600" />
        <Input placeholder="Value (e.g. 1.0.5)" value={val} onChange={e => setVal(e.target.value)} className="bg-zinc-950 border-zinc-800 text-white flex-1 focus-visible:ring-indigo-600" />
        <Button onClick={add} className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm"><Plus className="mr-2 h-4 w-4"/> Save Variable</Button>
      </div>
