    ...
    <MenuPrimitive.Portal>
      ...
      >
        <MenuPrimitive.Popup
          ...
          className={cn(
            // Layout
            "z-50 max-h-(--available-height) w-(--anchor-width) min-w-40",
            "origin-(--transform-origin) overflow-x-hidden overflow-y-auto",
            // Styling
            "rounded-xl bg-popover/95 backdrop-blur-xl p-1.5",
...
function DropdownMenuSubContent({
  ...
  return (
    <DropdownMenuContent
      ...
      className={cn(
        "w-auto min-w-[10rem] rounded-xl bg-popover/95 p-1.5 text-popover-foreground",
        "ring-1 ring-foreground/10 dark:ring-white/10",
        "shadow-xl shadow-black/12 dark:shadow-black/40",
        "backdrop-blur-xl",
        className
...
function DropdownMenuShortcut({
  ...
  return (
    <span
      ...
      className={cn(
        "ml-auto text-[11px] tracking-widest font-mono",
        "text-muted-foreground/50 group-focus/dropdown-menu-item:text-accent-foreground/70",
        className
      )}
      {...props}
    />
