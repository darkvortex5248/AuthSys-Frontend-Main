...
function Card({
  ...
  return (
    <div
      ...
      className={cn(
        // Base
        "group/card relative flex flex-col gap-4 overflow-hidden rounded-2xl",
        "bg-[var(--glass-bg)] backdrop-blur-xl py-5 text-sm text-[var(--card-foreground)]",
        // Border & shadow
        "ring-1 ring-[var(--border)] shadow-sm shadow-[var(--accent-opacity-8)]",
        // Size variants
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-2xl border-t border-[var(--border)]",
        "bg-[var(--muted)]/30 p-5 group-data-[size=sm]/card:p-4",
        "backdrop-blur-sm",
        className
      )}
      {...props}
...
// New: CardDivider for separating sections within a card
function CardDivider({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card-divider"
      className={cn("mx-5 h-px bg-[var(--border)]", className)}
      {...props}
    />
  )
}
