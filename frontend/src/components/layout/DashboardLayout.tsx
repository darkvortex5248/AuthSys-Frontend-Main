    ...
    <div className="flex h-screen bg-zinc-950 text-zinc-100 overflow-hidden">
      ...
      <aside className="w-64 border-r border-zinc-800 bg-zinc-950 flex flex-col flex-shrink-0">
        <div className="p-6 border-b border-zinc-800 flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 to-purple-400 tracking-tight">AuthSys</h1>
        </div>
        
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
          <Link href="/dashboard" className={`flex items-center gap-3 px-3 py-2 text-sm rounded-md transition-colors ${pathname === '/dashboard' ? 'bg-indigo-600/10 text-indigo-400 font-medium' : 'hover:bg-zinc-900 text-zinc-400 hover:text-zinc-100'}`}>
            <Home size={18} /> Overview
          </Link>
          <Link href="/apps" className={`flex items-center gap-3 px-3 py-2 text-sm rounded-md transition-colors ${pathname === '/apps' ? 'bg-indigo-600/10 text-indigo-400 font-medium' : 'hover:bg-zinc-900 text-zinc-400 hover:text-zinc-100'}`}>
          ...
          {activeAppId && (
            <div className="mt-8">
              ...
              {appNav.map((item) => {
                ...
                return (
                  <Link key={item.name} href={item.href} className={`flex items-center gap-3 px-3 py-2 text-sm rounded-md transition-colors ${isActive ? 'bg-indigo-600/10 text-indigo-400 font-medium' : 'hover:bg-zinc-900 text-zinc-400 hover:text-zinc-100'}`}>
