  ...
  if (!isReady) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-[#08080F]">
        <div className="w-12 h-12 border-4 border-[#d97757] border-t-transparent rounded-full animate-spin mb-4"></div>
    ...
    <div className="flex h-screen bg-zinc-950 text-zinc-100 overflow-hidden">
      ...
      <aside className="w-64 border-r border-zinc-800 bg-zinc-950 flex flex-col flex-shrink-0">
        <div className="p-6 border-b border-zinc-800 flex items-center gap-3">
          <div className="h-8 w-8 bg-[#d97757] rounded-lg flex items-center justify-center">
            <ShieldAlert size={18} className="text-white" />
          </div>
          <h1 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[#d97757] to-[#d97757] tracking-tight">AuthSys</h1>
        </div>
        
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
          <Link href="/dashboard" className={`flex items-center gap-3 px-3 py-2 text-sm rounded-md transition-colors ${pathname === '/dashboard' ? 'bg-[#d97757]/10 text-[#d97757] font-medium' : 'hover:bg-zinc-900 text-zinc-400 hover:text-zinc-100'}`}>
            <Home size={18} /> Overview
          </Link>
          <Link href="/apps" className={`flex items-center gap-3 px-3 py-2 text-sm rounded-md transition-colors ${pathname === '/apps' ? 'bg-[#d97757]/10 text-[#d97757] font-medium' : 'hover:bg-zinc-900 text-zinc-400 hover:text-zinc-100'}`}>
          ...
          {activeAppId && (
            <div className="mt-8">
              ...
              {appNav.map((item) => {
                ...
                return (
                  <Link key={item.name} href={item.href} className={`flex items-center gap-3 px-3 py-2 text-sm rounded-md transition-colors ${isActive ? 'bg-[#d97757]/10 text-[#d97757] font-medium' : 'hover:bg-zinc-900 text-zinc-400 hover:text-zinc-100'}`}>
