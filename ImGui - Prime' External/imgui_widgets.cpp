...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (it_anim == anim.end())
    {
        ...
        it_anim = anim.find(id);
        it_anim->second.hover_alpha = 0.0f;
    }

    float target_hover = hovered ? 1.0f : 0.0f;
    it_anim->second.hover_alpha = ImLerp(it_anim->second.hover_alpha, target_hover, g.IO.DeltaTime * 10.0f);
    ...
    // Button background with smooth hover glow
    ImU32 bg_col = GetColorU32(it_anim->second.outline, 1.0f + it_anim->second.hover_alpha * 0.2f);
    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, bg_col, ImStyle::button::rounding);

    invisible_shadow(1.f, bb.Min, bb.Max, GetColorU32(ImStyle::general_color, 0.5f + it_anim->second.hover_alpha * 0.5f), 500);
    ...
    ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (bb.GetWidth() - CalcTextSize(label).x) / 2, bb.Max.y - CalcTextSize(label).y - (bb.GetHeight() - CalcTextSize(label).y) / 2), GetColorU32(ImStyle::text::text_active), label);
...
bool ImGui::Tab(const char* icon, const char* tabname, int* tab, int number, int total_tabs, float total_region_width, float alpha) 
{
    ...
    float target_hover = hovered ? 1.0f : 0.0f;
    tabAnim.hover_alpha = ImLerp(tabAnim.hover_alpha, target_hover, g.IO.DeltaTime * 10.0f);
    ...
    // Render tab background with hover glow
    window->DrawList->AddRectFilled(total_bb.Min + ImVec2(0, total_bb.GetHeight() - 3), total_bb.Max, ImGui::GetColorU32(tabAnim.background_color, 1.0f + tabAnim.hover_alpha * 0.2f), 6.0f);
    window->DrawList->AddShadowRect(total_bb.Min - ImVec2(0, 3), total_bb.Max, ImGui::GetColorU32(tabAnim.background_color), 10.f + tabAnim.hover_alpha * 5.0f, ImVec2(0, 5), 0, 12.0f);

    if (is_active) {
        float shimmer_height = total_bb.GetHeight() * 0.6f;
        float shimmer_y_offset = (total_bb.GetHeight() - shimmer_height) * 0.5f;
        ...
        float shimmer_end = total_bb.GetWidth() + 30.0f;
    ...
    window->DrawList->AddText(total_bb.Min + ImVec2(15, (total_bb.GetHeight() - ImGui::CalcTextSize(icon).y) * 0.5f), ImColor(255, 255, 255, 255) - ImGui::GetColorU32(tabAnim.text_color_offset), icon);
    ImGui::PopFont();

    ImGui::PushFont(poppins_reg);
    window->DrawList->AddText(
        total_bb.Min + ImVec2((total_bb.GetWidth() - ImGui::CalcTextSize(tabname).x) * 0.5f, (total_bb.GetHeight() - ImGui::CalcTextSize(tabname).y) * 0.5f),
...
bool ImGui::TextButton(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    PushFont(fonts::icon);
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text, 1.0f + it_anim->second.hover_alpha * 0.2f));
    RenderTextClipped(bb.Min, bb.Max, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    
    // Add a subtle glow behind the text when hovered
    if (it_anim->second.hover_alpha > 0.01f)
    {
        window->DrawList->AddShadowRect(bb.Min, bb.Max, GetColorU32(ImStyle::general_color, it_anim->second.hover_alpha * 0.3f), 15.0f * it_anim->second.hover_alpha, ImVec2(0,0));
    }
...
struct check_state
{
    ...
    ImVec4 rect_color;
    float alpha;
    float hover_alpha; // Changed from hover_scale to hover_alpha
...
bool ImGui::Checkbox(const char* label, const char* description, bool* v)
{
    ...
    if (it_anim == anim.end())
    {
        ...
        it_anim = anim.find(id);
        it_anim->second.hover_alpha = 0.0f; // Initialize hover_alpha
    }
    ...
    float target_hover = hovered ? 1.0f : 0.0f;
    state.hover_alpha = ImLerp(state.hover_alpha, target_hover, g.IO.DeltaTime * 10.0f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    // Checkbox background highlight with smooth hover glow
    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImGui::GetColorU32(it_anim->second.rect_color, 0.1f + state.hover_alpha * 0.05f), ImStyle::button::rounding, 0);
    ...
    window->DrawList->AddRect(box_min, box_max, ImGui::GetColorU32(ImStyle::elements::background_widget, 0.7f + state.hover_alpha * 0.3f), 3.f);
    window->DrawList->AddRectFilled(box_min, box_max, ImGui::GetColorU32(ImStyle::elements::background_widget, 0.5f + state.hover_alpha * 0.2f), 3.f);
    ...
    ImVec2 box_center = (box_min + box_max) * 0.5f;
    box_center.y += bounce_offset_y;

    if (*v || scale > 0.001f)
        ImGui::RenderCheckMark(
            window->DrawList,
            box_center - ImVec2(square_size * 0.275f, square_size * 0.275f) * ease_scale,
