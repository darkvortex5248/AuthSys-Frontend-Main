...
export default function DeviceAppsPage() {
  ...
  const fetchApps = useCallback(async () => {
    try {
      const res = await api.get<DeviceApp[]>('/developer/device-apps');
      setApps(res.data || []);
    } catch {
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      await api.post('/developer/device-apps', { name: newName, max_devices: newMaxDevices });
      toast.success('Device app created');
      setShowCreate(false);
      setNewName('');
      setNewMaxDevices(50);
      fetchApps();
    } catch {
      toast.error('Failed to create device app');
    } finally {
      setCreating(false);
    }
  };

  const handleDelete = async (app: DeviceApp) => {
    if (!await confirm({ title: 'Delete device app?', message: `This will also remove all ${app.device_count} devices.`, confirmLabel: 'Delete', cancelLabel: 'Cancel', variant: 'danger' })) return;
    try {
      await api.delete(`/developer/device-apps/${app.id}`);
      toast.success('Device app deleted');
      fetchApps();
    } catch {
  ...
  const handleRegen = async (app: DeviceApp) => {
    ...
    try {
      const res = await api.post(`/developer/device-apps/${app.id}/regenerate-secret`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-[var(--foreground)]">Device Apps</h2>
          <p className="text-sm text-[var(--muted-foreground)] mt-0.5">Create device apps for your EXEs — no login required</p>
        </div>
      ...
      {showCreate && (
        <div className="glass-card rounded-2xl p-6 border border-white/5 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em]">App Name</label>
            ...
            <div>
              <label className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em]">Max Devices (0=∞)</label>
      ...
      {apps.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-[var(--foreground)] font-semibold">No device apps yet</p>
          ...
          <table className="w-full text-left border-collapse min-w-[700px]">
            <thead>
              <tr className="border-b border-white/5 bg-white/5">
                ...
                <th className="px-6 py-4 text-[10px] font-bold text-[var(--muted-foreground)] uppercase tracking-widest">Device Secret</th>
              ...
              {apps.map(app => (
                <tr key={app.id} className="hover:bg-white/[0.02] transition-colors">
                  <td className="px-6 py-4">
                    <div>
                      <p className="text-sm font-bold text-[var(--foreground)]">{app.name}</p>
                      <p className="text-[10px] text-[var(--muted-foreground)] mt-0.5">Created {app.created_at ? new Date(app.created_at).toLocaleDateString() : '-'}</p>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <code className="text-xs font-mono text-[var(--primary)] bg-[var(--primary)]/8 px-2 py-1 rounded-lg truncate max-w-[180px] block">
                        {app.device_secret.substring(0, 20)}...
                      </code>
                      <button onClick={() => copySecret(app.device_secret)}
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${
                      app.is_active ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'
                    }`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${app.is_active ? 'bg-emerald-500' : 'bg-red-500'}`} />
                      {app.is_active ? 'Active' : 'Disabled'}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-sm font-bold text-[var(--foreground)]">{app.device_count}</td>
                  <td className="px-6 py-4 text-sm text-[var(--muted-foreground)]">{app.max_devices >= 999999 ? '∞' : app.max_devices}</td>
                  <td className="px-6 py-4">
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => handleRegen(app)}
