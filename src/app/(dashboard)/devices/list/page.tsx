...
export default function DevicesListPage() {
  ...
  const fetchDevices = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get<Device[]>(`/developer/device-apps/${selectedAppId}/devices`);
      setDevices(res.data || []);
    } catch {
  ...
  const handleAction = async (deviceId: number, action: string) => {
    ...
    try {
      ...
      const res = await api.post(`/developer/device-apps/${selectedAppId}/devices/${deviceId}/${action}`);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-[var(--muted-foreground)] mt-0.5">Manage devices registered to your apps</p>
        </div>
        <select
          value={selectedAppId ?? ''}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-[var(--muted-foreground)] mt-1">Launch your EXE with the device secret to register</p>
        </div>
      ) : (
