...
export const description: string = "Edit one of your applications";
...
export const execute: Execute = async (ctx, bot) => {
  ...
  for (let i = 0; i < applications.length; i += 2) {
    ...
    if (app2) {
      keyboard.text(app1.name, `editseller:${app1.sellerkey}`).text(app2.name, `editseller:${app2.sellerkey}`);
    } else {
      keyboard.text(app1.name, `editseller:${app1.sellerkey}`);
    }
  ...
  await ctx.reply("Please click on one of your applications to edit it.", { reply_markup: keyboard });
}