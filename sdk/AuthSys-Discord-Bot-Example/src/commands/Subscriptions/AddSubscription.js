...
module.exports = {
    ...
    async execute(interaction) {
        ...
        let json = await config.api("/add-subscription", { name: subname, level: parseInt(sublevel) }, sellerkey)
        if (json.status === "success") {
            interaction.editReply({ embeds: [new EmbedBuilder().setTitle(json.detail || json.message).setColor(Colors.Green).setTimestamp()], ephemeral: ephemeral })
        } else {
