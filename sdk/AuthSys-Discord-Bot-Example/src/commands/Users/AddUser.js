...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let body = { username: user, subscription: sub, expiry: parseInt(expires), hwidAffected: hwidAffected };
    if (pass) body.password = pass;

    let json = await config.api("/add-user", body, sellerkey)
    if (json.status === "success") {
      ...
      interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setTitle(json.detail || json.message)
            .addFields([
              {
                name: "Note:",
