...
module.exports = {
    ...
    async execute(interaction) {
        ...
        let json = await config.api("/get-user-variable", { username: user, var_name: name }, sellerkey)
        if (json.status === "success") {
            interaction.editReply({ embeds: [new EmbedBuilder().setTitle(`User variable successfully retrieved`).addFields([{ name: 'Variable data:', value: `\`${json.data || json.response || json.message}\`` }]).setColor(Colors.Blue).setTimestamp()], ephemeral: ephemeral })
        } else {
