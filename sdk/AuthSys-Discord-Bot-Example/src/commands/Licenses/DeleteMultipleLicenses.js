...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            const results = [];
            for (const key of keyList) {
                const json = await config.api("/delete-key", { key_value: key, userToo: userToo }, sellerkey)
                results.push(json)
            }
            ...
            if (allSuccess) {
                return interaction.editReply({
                    embeds: [
                        new EmbedBuilder()
                            .setTitle("Successfully deleted all specified keys")
            ...
            return interaction.editReply({
                embeds: [
                    new EmbedBuilder()
                        .setTitle("Some keys could not be deleted")
