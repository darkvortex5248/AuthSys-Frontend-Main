...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!ip && !hwid) {
      return interaction.editReply({
        embeds: [new EmbedBuilder().setDescription(`You need to either define hwid or ip paramater. Please try again defining one of these paramaters..`).setColor(Colors.Red).setTimestamp()],
        ephemeral: ephemeral,
      });
    }
    if (ip && hwid) {
      return interaction.editReply({
        embeds: [new EmbedBuilder().setDescription(`Please only define one paramater per command..`).setColor(Colors.Red).setTimestamp()],
        ephemeral: ephemeral,
      });
    }
    ...
    if (hwid) body.hwid = hwid;
        ...
        let json = await config.api("/add-blacklist", body, sellerkey)
    if (json.status === "success") {
      interaction.editReply({
        embeds: [new EmbedBuilder().setTitle(json.detail || json.message).setColor(Colors.Green).setTimestamp()],
        ephemeral: ephemeral,
      });
    } else {
      interaction.editReply({
        embeds: [new EmbedBuilder().setTitle(json.detail || json.message).addFields([{ name: "Note:", value: `Your seller key is most likely invalid. Change your seller key with \`/add-application\` command.` }]).setColor(Colors.Red).setTimestamp().setFooter({ text: "AuthSys Discord Bot" })],
        ephemeral: ephemeral,
      });
    }
  },
