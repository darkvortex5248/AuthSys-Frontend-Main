    ...
    public partial class LoginForm : Form
    {
        ...
        private async void InitializeAuth()
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show($"Update required: {result.Message}", "Update Required", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    Application.Exit();
                    return;
                }
            }
        ...
        private async Task Login()
        {
            try
            {
                ...
                if (result.Success)
                {
                    new DashboardForm(_auth, result.Username).Show();
                    this.Hide();
                }
            }
            catch (AuthSysException ex)
        ...
        public DashboardForm(AuthSys.AuthSys auth, string username)
        {
            ...
            btnVerify.Click += async (s, e) =>
            {
                try
                {
                    ...
                    txtStatus.Text = $"Valid: {result.Valid}\nUsername: {result.Username}\nExpires: {result.ExpiresAt}";
                }
                catch (AuthSysException ex)
                {
        ...
        public RegisterForm(AuthSys.AuthSys auth)
        {
            ...
            btnRegister.Click += async (s, e) =>
            {
                try
                {
                    ...
                    MessageBox.Show(result.Message, "Registration", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    this.Close();
                }
                catch (AuthSysException ex)
                {
